...
5. Use `QUÉT QUÁI / HP`, inspect Name/RoleID/ResID/HP/Class, add the intended monsters, then assign normal or boss groups.
...
- `alive (HP>0) -> HP=0/IsDeath` emits exactly one event.
...
The scanner and semantic AutoFight action run only inside the existing WH_GETMESSAGE bridge callback. Exact RVAs are guarded by the frozen PE tuple and byte signatures. The controller stores only copied values in shared memory; it never retains client object pointers.

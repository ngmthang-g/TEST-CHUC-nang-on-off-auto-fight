...
constexpr wchar_t kTitle[] = L"Thần Long AutoFight Test v0.1.2";
...
constexpr int IDC_AUTOFIGHT_ON = 110;
constexpr int IDC_AUTOFIGHT_OFF = 111;
constexpr int IDC_STATUS = 120;
constexpr int IDC_LOG = 121;
...
public:
    ...
    bool Attach(const GameClient& game, std::wstring& error) {
        ...
        if (!localDll_) {
            error = L"Thiếu ThanLongAutoFightTestBridge.dll cạnh EXE";
            Close();
            return false;
        }
    ...
    bool Call(Command command, Response& out, std::wstring& error, DWORD timeoutMs = 2500) {
        ...
        error = L"Bridge timeout; không tự retry";
        return false;
    }

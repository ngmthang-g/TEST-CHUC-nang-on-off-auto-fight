...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference='Stop'; $files=@('src/controller.cpp','src/bridge.cpp','src/protocol.h'); $s=($files|%%{Get-Content $_ -Raw}) -join [Environment]::NewLine;" ^
  ...
  "$bridge=Get-Content 'src/bridge.cpp' -Raw; foreach($x in @('LuaSystemManager','ExecuteFunction','TopIcon','AutoTrainClick','AutoStopClick','SetAutoFight')){if($bridge -notmatch [regex]::Escape($x)){throw ('Missing AutoFight semantic token '+$x)}};" ^
  "$proto=Get-Content 'src/protocol.h' -Raw; foreach($x in @('ReadState = 1','StartAutoFight = 5','StopAutoFight = 6')){if($proto -notmatch [regex]::Escape($x)){throw ('Protocol missing '+$x)}};" ^
...
zig c++ -target x86_64-windows-gnu -O2 -std=c++17 -Wall -Wextra -Werror -municode -static -s ^
  ...
  -o dist\ThanLongAutoFightTest_v0.1.0.exe
if errorlevel 1 exit /b 1

echo [7/7] Done.
echo BUILD THANH CONG - AUTOFIGHT TEST v0.1.0
